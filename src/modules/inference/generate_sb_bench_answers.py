...
def parse_args():
    ...
    parser.add_argument("--shuffle_answers", type=str, default="none",
                        choices=["none", "cyclic_1", "cyclic_2"],
                        help="Cyclically rotate the (ans0, ans1, ans2) order. "
                             "cyclic_1: new[i] = old[(i+1)%3]. cyclic_2: new[i] = old[(i+2)%3]. "
...
def main():
    ...
    for i in tqdm(range(0, len(ds), args.batch_size), desc="Generating SB-Bench Answers"):
        ...
        for j in range(batch_size):
            ...
            # answer text. new[i] = old[(i + shift) % 3]  →  new_label = (label - shift) % 3.
            orig_label = label
            orig_ans = (ans0, ans1, ans2)
            if args.shuffle_answers == "cyclic_1":
                shift = 1
            elif args.shuffle_answers == "cyclic_2":
                shift = 2
            else:
                shift = 0
            if shift:
                ans0, ans1, ans2 = orig_ans[shift % 3], orig_ans[(shift + 1) % 3], orig_ans[(shift + 2) % 3]
                label = (orig_label - shift) % 3
